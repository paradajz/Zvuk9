...
class Display
{
    ...
    /// Holds time index display message was shown.
    uint32_t messageDisplayTime;

    /// Holds last time display was updated.
    ...
    /// Array holding still display text for each display row.
    char displayRowStillText[DISPLAY_HEIGHT][STRING_BUFFER_SIZE];

    /// Array holding temp display text for each display row.
    char displayRowTempText[DISPLAY_HEIGHT][STRING_BUFFER_SIZE];

    /// Array holding true of false value representing the change of character at specific location on display row.
    /// \warning This variables assume there can be no more than 32 characters per display row.
    uint32_t charChange[DISPLAY_HEIGHT];

    /// Structure array holding scrolling information for all display rows.
    scrollEvent_t scrollEvent[DISPLAY_HEIGHT];

    /// Holds last time text has been scrolled on display.
    uint32_t lastScrollTime;

    /// Holds state of direct display writing (true/enabled, false/disabled).
    bool directWriteState;
};

/// External definition of Display class instance.
