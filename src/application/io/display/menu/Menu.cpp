...
/// Currently active position in menu.
int16_t menuHierarchyPosition;

/// Total number of items in current menu layout.
uint8_t menuSize;

/// Flag indicating whether or not menu item function is running.
bool functionRunning;

/// Total number of items in current menu hierarchy.
uint8_t menuHierarchyItems;

/// Array holding indexes of items in current menu hierarchy.
/// Due to simple menu design, maximum number of indexes in hierarchy is 9.
uint8_t itemIndex[9];

/// Pointer to menu layout.
...
/// Shows initial screen for specified menu type.
...
/// Retrieve number of items in current menu hierarchy and store their indexes.
...
/// Displays all items for current menu hierarchy on display.
...
/// Checks if menu is currently active.
...
/// Handles switching of menu screens when options are selected.
