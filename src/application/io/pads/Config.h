...
/// Time in milliseconds after which X/Y values are being read after pad has been pressed.
/// Initial X/Y values tend to be unstable.
#define XY_READ_DELAY 3

/// Time in milliseconds during which aftertouch reading is ignored after pad has been pressed.
/// Used to avoid incorrect aftertouch activation on initial press.
#define AFTERTOUCH_READ_DELAY 200

/// Time in milliseconds during which pressure reading is ignored after X or Y has changed value.
#define PRESSURE_IGNORE_XY_CHANGE 2

/// Time in milliseconds after which notes from pad are sent once X and Y MIDI messages have been sent.
#define PAD_NOTE_SEND_DELAY (XY_READ_DELAY + 3)

/// Size of pad note buffer in bytes.
...
/// Value used to reset last X, Y and aftertouch values so that new value can always differ from the last one.
#define DEFAULT_XY_AT_VALUE 255

/// Value used to reset last X and Y values for pitch bend so that new value can always differ from the last one.
#define DEFAULT_PITCH_BEND_VALUE 16000

/// Value used to reset last initial X and Y values so that new value can always differ from the last one.
#define DEFAULT_INITIAL_XY_VALUE 16000

/// Value range for pitch bend type 1.
...
/// Minimum and maximum MIDI pitch bend values.
/// @{
...
/// When Pitch Bend Type 2 is used, 7% of pad surface is used for full PB range in positive, and 7% in negative direction.
...
/// Area where pitch bend type 2 is zero (5% of surface).
...
/// Percentage used to calculate upper pressure limit on medium pressure sensitivity to send velocity.
...
/// Percentage used to calculate upper pressure limit on hard pressure sensitivity to send velocity.
...
/// Time in milliseconds after which aftertouch value needs to change by AFTERTOUCH_SEND_TIMEOUT_STEP in order to send new value.
/// Used to debounce rapidly changing aftertouch values.
#define AFTERTOUCH_SEND_TIMEOUT 3

/// Difference between current and previous aftertouch reading after AFTERTOUCH_SEND_TIMEOUTms used to detect value change.
#define AFTERTOUCH_SEND_TIMEOUT_STEP 5

/// Time in milliseconds after which X/Y value needs to change by XY_SEND_TIMEOUT_STEP in order to send new value.
/// Used to debounce rapidly changing X/Y values.
#define XY_SEND_TIMEOUT 5

/// Minimum MIDI pressure (velocity) value necessary to send X/Y values (after initial values have been sent).
/// Used to avoid unstable X/Y values on low pressure.
#define XY_MIN_PRESSURE_PRESSED 25

/// Difference between current and previous aftertouch reading after XY_SEND_TIMEOUT used to detect value change.
#define XY_SEND_TIMEOUT_STEP 3

/// Number of zones per pad on X-axis for which calibration data is stored.
#define PRESSURE_CALIBRATION_X_ZONES 5

/// Range of values in single pressure zone on X axis.
#define PRESSURE_CALIBRATION_MAX_X_ZONE_VALUE ((1023 / PRESSURE_CALIBRATION_X_ZONES) + 1)

/// Number of zones per pad on Y-axis for which calibration data is stored.
#define PRESSURE_CALIBRATION_Y_ZONES 5

/// Range of values in single pressure zone on Y axis.
#define PRESSURE_CALIBRATION_MAX_Y_ZONE_VALUE ((1023 / PRESSURE_CALIBRATION_Y_ZONES) + 1)

/// Total number of zones per pad for which calibration data is stored.
#define PRESSURE_CALIBRATION_ZONES (PRESSURE_CALIBRATION_X_ZONES * PRESSURE_CALIBRATION_Y_ZONES)

/// Time in seconds after which last read pressure value on certain pressure zone is considered calibrated value in calibration mode.
#define PRESSURE_ZONE_CALIBRATION_TIMEOUT 5

/// Percentage used to calculate lower pressure limit to send aftertouch.
...
/// Percentage used to calculate upper pressure limit to send aftertouch.
