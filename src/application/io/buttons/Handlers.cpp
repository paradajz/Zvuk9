...
/// Handler for on/off buttons (split, notes, aftertouch, X and Y).
...
/// Handler for transport control buttons (play, stop and record).
/// @param [in] id      Button index.
/// @param [in] state   Button state (true/pressed, false/released).
void handleTransportControl(uint8_t id, bool state)
{
    ...
    case BUTTON_TRANSPORT_PLAY:
        ...
        switch (getTransportControlMode())
        {
        case transportCC:
...
void handleUpDown(uint8_t id, bool state)
{
    ...
    if (getButtonState(BUTTON_OCTAVE_DOWN) && getButtonState(BUTTON_OCTAVE_UP))
    {
#ifdef DEBUG
        ...
        setButtonEnableState(BUTTON_OCTAVE_UP, false);
        setButtonEnableState(BUTTON_OCTAVE_DOWN, false);
    }
    else
    {
        ...
        case false:
            ...
            else if (getButtonState(BUTTON_ON_OFF_NOTES))
            {
                ...
                if (!state)
                {
                    ...
                    case valueChanged:
                        direction ? leds.setLEDstate(LED_OCTAVE_UP, ledStateOff) : leds.setLEDstate(LED_OCTAVE_DOWN, ledStateOff);
                        setButtonEnableState(BUTTON_ON_OFF_NOTES, false);
...
void handleTonic(uint8_t id, bool state)
{
    ...
    note_t note = getNoteFromButton(id);
...
/// Handler for button located on program encoder.
/// @param [in] id      Button index.
/// @param [in] state   Button state (true/pressed, false/released).
...
/// Handler for button located on preset encoder.
/// @param [in] id      Button index.
/// @param [in] state   Button state (true/pressed, false/released).
